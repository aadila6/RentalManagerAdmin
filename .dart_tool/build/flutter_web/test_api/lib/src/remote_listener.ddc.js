define(['dart_sdk', 'packages/stream_channel/stream_channel', 'packages/test_api/src/suite_channel_manager', 'packages/test_api/src/backend/closed_exception', 'packages/async/async', 'packages/term_glyph/src/generated/ascii_glyph_set', 'packages/test_api/src/backend/metadata'], function(dart_sdk, packages__stream_channel__stream_channel, packages__test_api__src__suite_channel_manager, packages__test_api__src__backend__closed_exception, packages__async__async, packages__term_glyph__src__generated__ascii_glyph_set, packages__test_api__src__backend__metadata) {
  ...
  var $add = dartx.add;
  var $map = dartx.map;
  var StreamChannelControllerOfObjectL = () => (StreamChannelControllerOfObjectL = dart.constFn(stream_channel_controller.StreamChannelController$(core.Object)))();
  ...
  var GroupEntryLToMapL = () => (GroupEntryLToMapL = dart.constFn(dart.fnType(core.Map, [group_entry.GroupEntry])))();
  var StateLToNullN = () => (StateLToNullN = dart.constFn(dart.fnType(core.Null, [state.State])))();
  var AsyncErrorLToNullN = () => (AsyncErrorLToNullN = dart.constFn(dart.fnType(core.Null, [async.AsyncError])))();
  ...
  var L1 = "package:test_api/src/remote_listener.dart";
  var L0 = "file:///private/var/folders/kv/jfkyvgcx1q3_7nwhmrnqw7z80000gn/T/scratch_spacenntfA9/packages/test_api/src/remote_listener.dart";
  dart.defineLazy(CT, {
    get C0() {
  ...
  remote_listener.RemoteListener = class RemoteListener extends core.Object {
    static start(getMain, opts) {
      ...
      let controller = new (StreamChannelControllerOfObjectL()).new({allowForeignErrors: false, sync: true});
      ...
      let spec = new async._ZoneSpecification.new({print: dart.fn((_, __, ___, line) => {
          ...
          channel.sink.add(new (IdentityMapOfStringL$StringL()).from(["type", "print", "line", line]));
        }, ZoneLAndZoneDelegateLAndZoneL__ToNullN())});
      async.Stream.fromIterable([]).listen(dart.fn(_ => {
      }, dynamicToNullN())).cancel();
        ...
        new stack_trace_formatter.StackTraceFormatter.new().asCurrent(core.Null, dart.fn(() => {
          async.runZoned(FutureOfNullN(), dart.fn(() => async.async(core.Null, function*() {
            ...
            if (!core.Function.is(main)) {
              ...
              return;
            } else if (!VoidToLdynamic().is(main)) {
            ...
            if (!dart.equals(dart.dsend(message, '_get', ["type"]), "initial")) dart.assertFailed(null, L0, 95, 18, "message['type'] == 'initial'");
            queue.rest.listen(dart.fn(message => {
              if (dart.equals(dart.dsend(message, '_get', ["type"]), "close")) {
                controller.local.sink.close();
                return;
              }
              if (!dart.equals(dart.dsend(message, '_get', ["type"]), "suiteChannel")) dart.assertFailed(null, L0, 103, 20, "message['type'] == 'suiteChannel'");
            ...
            let declarer = new declarer$.Declarer.new({metadata: metadata, platformVariables: LinkedHashSetOfStringL().from(core.Iterable.as(dart.dsend(message, '_get', ["platformVariables"]))), collectTraces: core.bool.as(dart.dsend(message, '_get', ["collectTraces"])), noRetry: core.bool.as(dart.dsend(message, '_get', ["noRetry"]))});
            ...
            yield declarer.declare(dart.dynamic, VoidToLdynamic().as(main));
            ...
            }, VoidToNullN()), {zoneValues: new _js_helper.LinkedMap.from([C0 || CT.C0, declarer])});
          }), VoidToFutureLOfNullN()), {onError: dart.fn((error, stackTrace) => {
              remote_listener.RemoteListener._sendError(channel, error, stackTrace, verboseChain);
            }, dynamicAndStackTraceLToNullN()), zoneSpecification: spec});
        }, VoidToNullN()));
      }, VoidToNullN()));
    ...
    [_serializeTest](channel, test, groups) {
      ...
      testChannel.stream.listen(dart.fn(message => {
        if (!dart.equals(dart.dsend(message, '_get', ["command"]), "run")) dart.assertFailed(null, L0, 214, 14, "message['command'] == 'run'");
    ...
    [_runLiveTest](liveTest, channel) {
      channel.stream.listen(dart.fn(message => {
        if (!dart.equals(dart.dsend(message, '_get', ["command"]), "close")) dart.assertFailed(null, L0, 231, 14, "message['command'] == 'close'");
        liveTest.close();
      }, dynamicToNullN()));
      liveTest.onStateChange.listen(dart.fn(state => {
        channel.sink.add(new (IdentityMapOfStringL$StringL()).from(["type", "state-change", "status", state.status.name, "result", state.result.name]));
      }, StateLToNullN()));
      liveTest.onError.listen(dart.fn(asyncError => {
        channel.sink.add(new (IdentityMapOfStringL$ObjectL()).from(["type", "error", "error", remote_exception.RemoteException.serialize(asyncError.error, stack_trace_formatter.StackTraceFormatter.current.formatStackTrace(asyncError.stackTrace, {verbose: liveTest.test.metadata.verboseTrace}))]));
      }, AsyncErrorLToNullN()));
      liveTest.onMessage.listen(dart.fn(message => {
        if (this[_printZone$] != null) this[_printZone$].print(message.text);
        channel.sink.add(new (IdentityMapOfStringL$StringL()).from(["type", "message", "message-type", message.type.name, "text", message.text]));
      }, MessageLToNullN()));
      async.runZoned(core.Null, dart.fn(() => {
        liveTest.run().then(dart.void, dart.fn(_ => channel.sink.add(new (IdentityMapOfStringL$StringL()).from(["type", "complete"])), dynamicTovoid()));
      }, VoidToNullN()), {zoneValues: new _js_helper.LinkedMap.from([C1 || CT.C1, channel])});
    }
  };
  ...
  dart.setLibraryUri(remote_listener.RemoteListener, L1);
